...
class SimpleSwitchNX(app_manager.RyuApp):
    ...
    def add_switch(self, ev):
        ...
        self.all_datapaths.append(switch.dp)
    ...
    def packet_in_handler(self, ev):
        ...
        if dst in self.net.nodes() and src in self.net.nodes():
            ...
            # Get all ports from all datapaths (Switch)
            for curr_dp in self.all_datapaths:
                curr_parser = curr_dp.ofproto_parser
                available_ports = [dp_port for dp_port in curr_dp.ports
                ...
                if ports_to_send:
                    for p_flood in ports_to_send:
                        actions.append(curr_parser.OFPActionOutput(p_flood))
                        
                    data = None
                    if msg.buffer_id == ofproto.OFP_NO_BUFFER:
                        data = msg.data
                    out = curr_parser.OFPPacketOut(datapath=curr_dp,
                                              buffer_id=msg.buffer_id,
                                              in_port=65535,
                                              actions=actions,
                                              data=data)
                    self.logger.debug('[packet_in_handler]: Sending message to [dpid:%s][port=%s]',
                              dpid_lib.dpid_to_str(curr_dp.id), str(p_flood))
                    datapath.send_msg(out)
                    ports_sent.append((dpid_lib.dpid_to_str(curr_dp.id), str(p_flood)))
